...
def make_block_entry(di, block):
    ...
    output.update(
        make_entry(make_func, description=super_description, params=block.params)
    )
    return output
        ...
        for pydoc_file in pydoc_files:
            if filter_str:
                filter_str2 = "::".join(
                    (
                        filter_str,
                        os.path.split(pydoc_file)[-1].split("_pydoc_template.h")[0],
                    )
                )
                docstrings_dict2 = {
                    k: v
                    for k, v in docstrings_dict.items()
                    if k.startswith(filter_str2)
                }
            else:
...
def argParse():
    ...
    parser.add_argument(
        "function",
        help="Operation to perform on docstrings",
